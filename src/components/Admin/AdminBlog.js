...
export default function AdminBlog() {
  ...
  async function dele() {
    try {
      ...
      const response = await fetch("http://localhost:8000/blog_delete", {
        method: "POST",
        headers: {
  ...
  async function insert() {
    try {
      ...
      const response = await fetch("http://localhost:8000/blog_insert", {
        method: "POST",
        headers: {
          "Content-Type": "application/json", // Make sure this is set correctly
        },
      ...
      .then((data) => {
        data.forEach((blog) => {
          temp = ` <div class="card text-center bg-transparent">
            <div class="card-body">
              <h4 class="card-title">${blog.blog_id}</h4>
